<script lang="ts">
    ...
    import { Flag, AlertTriangle, Loader2, UserX } from "lucide-svelte";
    ...
    let { open = $bindable(false), userId, userName = "this user", onClose, onReported }: Props = $props();

    type UserReportReason = 'harassment' | 'spam' | 'scam' | 'fake_profile' | 'impersonation' | 'inappropriate_content' | 'animal_abuse' | 'threats' | 'other';

    const reportReasonKeys: { value: UserReportReason; labelKey: string; descKey: string }[] = [
        { value: 'harassment', labelKey: 'user_report.reason_harassment', descKey: 'user_report.reason_harassment_desc' },
        { value: 'spam', labelKey: 'user_report.reason_spam', descKey: 'user_report.reason_spam_desc' },
        { value: 'scam', labelKey: 'user_report.reason_scam', descKey: 'user_report.reason_scam_desc' },
        { value: 'fake_profile', labelKey: 'user_report.reason_fake_profile', descKey: 'user_report.reason_fake_profile_desc' },
        { value: 'impersonation', labelKey: 'user_report.reason_impersonation', descKey: 'user_report.reason_impersonation_desc' },
        { value: 'inappropriate_content', labelKey: 'user_report.reason_inappropriate', descKey: 'user_report.reason_inappropriate_desc' },
        { value: 'animal_abuse', labelKey: 'user_report.reason_animal_abuse', descKey: 'user_report.reason_animal_abuse_desc' },
        { value: 'threats', labelKey: 'user_report.reason_threats', descKey: 'user_report.reason_threats_desc' },
        { value: 'other', labelKey: 'user_report.reason_other', descKey: 'user_report.reason_other_desc' },
    ];

    let selectedReasons = $state<UserReportReason[]>([]);
    let description = $state("");
    let isSubmitting = $state(false);

    function toggleReason(reason: UserReportReason) {
    ...
    async function handleSubmit() {
        if (selectedReasons.length === 0) {
            toast.error($_("user_report.select_reason"));
            return;
        }

        if (!auth.token) {
            toast.error($_("user_report.must_login"));
            return;
        }
        ...
        try {
            const mutation = `
                mutation ReportUser($userId: ID!, $reasons: [UserReportReason!]!, $description: String) {
                    reportUser(userId: $userId, reasons: $reasons, description: $description) {
                        id
                        reasons
                        status
                    }
            ...
            toast.success($_("user_report.success"));
            resetForm();
            onClose();
            onReported?.();
        } catch (e: any) {
            toast.error(e.message || $_("user_report.failed"));
        } finally {
            isSubmitting = false;
        }
    }
...
<Modal {open} onClose={handleClose} title={$_("user_report.title")}>
    <div class="space-y-5">
        <!-- Header Info -->
        <div class="flex items-center gap-3 pb-4 border-b border-gray-100 dark:border-gray-700">
            ...
            <div>
                <p class="text-sm text-gray-500 dark:text-gray-400">{$_("user_report.subtitle")}</p>
        ...
        <div role="group" aria-labelledby="report-reasons-label">
            <span id="report-reasons-label" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                {$_("user_report.why_reporting")} <span class="text-red-500">*</span>
                <span class="font-normal text-gray-500 dark:text-gray-400 ml-1">({$_("user_report.select_all")})</span>
            </span>
        ...
        <div role="group" aria-labelledby="report-description-label">
            <span id="report-description-label" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {$_("user_report.additional_details")} ({$_("common.optional")})
            </span>
            <RichTextEditor
                content={description}
                placeholder={$_("user_report.additional_placeholder")}
                maxLength={1000}
                minHeight="100px"
