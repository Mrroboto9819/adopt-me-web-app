                ...
                {#if !isRegistering || (isRegistering && step === 1)}
                    ...
                    <div>
                        ...
                        {#if isRegistering}
                            ...
                            <label class="flex items-start gap-3 cursor-pointer select-none mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <input
                                    type="checkbox"
                                    bind:checked={agreedToTerms}
                                    class="mt-0.5 w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-400 bg-white dark:bg-gray-800"
                                />
                                <span class="text-sm text-gray-600 dark:text-gray-300">
                                    {$t("auth.agree_terms")}
                                    <a href="/terms" target="_blank" class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">{$t("auth.terms_of_service")}</a>
                                    {$t("auth.and")}
                                    <a href="/privacy" target="_blank" class="text-indigo-600 dark:text-indigo-400 hover:underline font-medium">{$t("auth.privacy_policy")}</a>
                                </span>
                            </label>
                        {:else}
